...
def generate_Cij(K, N, STRUCTURE=None, SIGMA=1):
    ...
    if STRUCTURE is None:
        ...
        Cij = np.random.rand(N, N) < K / N * np.array(1 + SIGMA * cos_ij)
...
class Network:
    ...
    def update_inputs(self, Cij):
        ...
        for i in range(self.N):
            ...
            pres_E = np.nonzero(Cij[i, :NE])
            pres_I = np.nonzero(Cij[i, NE:]) 
            
            # sum over presynatptic neurons
            if i < NE:
                self.inputs[0][i] += np.sum(self.rates[pres_E])
                self.inputs[1][i] += np.sum(self.rates[pres_I])
            else:
                self.inputs[0][i] += np.sum(self.rates[pres_E])
                self.inputs[1][i] += np.sum(self.rates[pres_I])
             
        self.inputs[0][:NE] *= self.Jab[0][0]
        self.inputs[1][:NE] *= self.Jab[0][1]

        self.inputs[0][NE:] *= self.Jab[1][0]
        self.inputs[1][NE:] *= self.Jab[1][1]
