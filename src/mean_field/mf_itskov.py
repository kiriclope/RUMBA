...
def integrand(theta, u0, u1):
    return Phi(u_theta(theta, u0, u1))


def integrand2(theta, u0, u1):
    return Phi(u_theta(theta, u0, u1)) * np.cos(theta)


def m0_func(u0, u1):
    res, err = quad(integrand, 0, 2.0 * np.pi, args=(u0, u1), limit=500)
    return res / 1.0 / 2.0 / np.pi


def m1_func(u0, u1):
    res, err = quad(integrand2, 0, 2.0 * np.pi, args=(u0, u1), limit=500)
...
class MeanFieldSpec:
    ...
    def __init__(self, **kwargs):
        ...
        self.kappa_Jab = 2.0 * self.Jab * self.kappa
        ...
        self.initial_guess = np.random.rand(2* self.N_POP)
    ...
    def self_consistent_eqs(self, x):
        ...
        m0 = m0_func(u0, u1)
        u0_eq = u0 - (self.Iext + np.dot(self.Jab, m0))
        
        m1 = m1_func(u0, u1)
        u1_eq = u1 - np.dot(self.kappa_Jab, m1)
                
        eqs = np.array([u0_eq, u1_eq])
    ...
    def solve(self):
        ...
        while any( self.error > self.TOLERANCE ):
            ...
            self.initial_guess = np.random.rand(2 * self.N_POP) * 2.0 - 1.0
    ...
    def solutions(self):
        ...
        self.m0 = m0_func(self.u0, self.u1)
        self.m1 = m1_func(self.u0, self.u1)
    ...
    def bifurcation(self):
        ...
        self.kappas = np.linspace(0, 1, 20)
