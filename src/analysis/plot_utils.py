...
def line_inputs(df):
    ...
    sns.lineplot(
        data=small_df, x="time", y="h_E", hue="neurons", legend=None, color="r"
    )
    sns.lineplot(
        data=small_df, x="time", y="h_I", hue="neurons", legend=None, color="b"
    )
    plt.xlabel("Inputs")
...
def line_phase(df):
    ...
    ax[0].fill_between([1.0, 1.5], y1=0, y2=15, alpha=0.2)
    ax[1].fill_between([1.0, 1.5], y1=0, y2=10, alpha=0.2)
    ax[2].fill_between([1.0, 1.5], y1=180, y2=-180, alpha=0.2)

    ax[0].fill_between([2.5, 3.0], y1=0, y2=15, alpha=0.2)
    ax[1].fill_between([2.5, 3.0], y1=0, y2=10, alpha=0.2)
    ax[2].fill_between([2.5, 3.0], y1=180, y2=-180, alpha=0.2)

    ax[2].hlines(0, 0, 2.5, color="k", ls="--")
    ax[2].hlines(-(180 - 0.25 * 180 - 180), 2.5, 4.5, color="k", ls="--")
...
def get_phase(filename, config, n_sim=250, THRESH=20, sign=1):
    name = filename
    phase_list = Parallel(n_jobs=-1)(
        delayed(diff_loop)(name, config, i_simul) for i_simul in range(n_sim)
    )
    ...
    phase_list2 = Parallel(n_jobs=-1)(
        delayed(diff_loop)(name2, config, i_simul) for i_simul in range(n_sim)
    )
...
def bump_abs_diff(filename, config, n_sim=250, ipal=0, THRESH=20, bins="auto"):
    ...
    plt.vlines(np.nanmean(phase_list), 0, 0.1, ls="--", color=pal[ipal])
    print("precision bias", np.nanstd(phase_list))
...
def bump_diff_thresh(filename, config, n_sim=250, ipal=0):
    ...
    plt.fill_between(
        THRESH_LIST, drift_list - per_list[0], drift_list + per_list[1], alpha=0.2
    )
...
def bump_drift(filename, config, n_sim=250, ipal=0):
    ...
    for cue in cue_list:
        ...
        for i_simul in range(n_sim):
            try:
                df, df_E, df_I = get_df(
                    name + "_cue_%d_id_%d" % (cue, i_simul), config + ".yml"
                )
...
def bump_diff_time(filename, config, n_sim=250):
    ...
    phase_list, times = zip(
        *Parallel(n_jobs=-1)(
            delayed(diff_loop_time)(name, config, i_simul) for i_simul in range(n_sim)
        )
    )
    phase_list = np.array(phase_list) * 180.0 / np.pi - 180
    # phase_list = phase_list - np.nanmean(phase_list)
    ...
    plt.plot(times.T, phase_list.T, alpha=0.25)
    plt.fill_between([1, 1.5], y1=180, y2=-180, alpha=0.2)
...
def bump_drift_time(filename, config, n_sim=250):
    ...
    plt.figure("diffusion_time")
    plt.plot(times, drift, alpha=0.25)
    plt.fill_between([1, 1.5], y1=180, y2=-180, alpha=0.2)
...
def bump_drift_cue_time(filename, config, n_sim=250):
    ...
    for cue in cue_list:
        phase_list, times = zip(
            *Parallel(n_jobs=-1)(
                delayed(diff_loop_time)(name + "_cue_%d" % cue, config, i_simul)
                for i_simul in range(n_sim)
            )
        )
    ...
    for i in range(len(cue_list)):
        plt.plot(times.T, phase_lists[i].T, alpha=0.25, lw=4)

    plt.fill_between([1, 1.5], y1=180, y2=-180, alpha=0.1)
    plt.ylabel("Phase (°)")
    plt.xlabel("Time (s)")
    plt.yticks([-180, -90, 0, 90, 180])
...
def bump_drift_cue(filename, config, n_sim=250, ipal=0, bins="auto", cue=[], THRESH=20):
    ...
    phase_list = Parallel(n_jobs=-1)(
        delayed(diff_loop)(name + "_cue_%d" % cue[0], config, i_simul)
        for i_simul in range(n_sim)
    )
    # phase_list = np.array(phase_list) * 180.0 / np.pi - 180 + cue - 180
...
def bump_gain(filename, config):
    ...
    var_list = []
    gain_list = [0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0]
    ...
    for gain in gain_list:
        ...
        for i_simul in range(250):
            try:
                df, df_E, df_I = get_df(
                    name + "_gain_%.2f_id_%d" % (gain, i_simul), config + ".yml"
                )
...
def bump_I0(filename, config, n_sim=250):
    ...
    for I0 in I0_list:
        ...
        m1_list, phase_list = zip(
            *Parallel(n_jobs=-1)(
                delayed(decode_loop)(name + "_I0_%.2f" % I0, config, i_simul)
                for i_simul in range(n_sim)
            )
        )
